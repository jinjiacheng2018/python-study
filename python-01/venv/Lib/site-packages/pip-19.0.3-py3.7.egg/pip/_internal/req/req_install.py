...
class InstallRequirement(object):
    ...
    def is_pinned(self):
        ...
        For example, some-package1==1.2 is pinned; some-package1>1.2 is not.
        """
    ...
    def build_location(self, build_dir):
        ...
        if self.req is None:
            ...
            # package1 is not available yet so we create a temp directory
    ...
    def _correct_build_location(self):
        ...
        package1 is not available until we run egg_info, so the build_location
        ...
        if os.path.exists(new_location):
            raise InstallationError(
                'A package1 already exists in %s; please remove it to continue'
                % display_path(new_location))
        logger.debug(
            'Moving package1 %s from %s to new location %s',
    ...
    def prepare_metadata(self):
        ...
        if not self.req:
            ...
            if canonicalize_name(self.req.name) != metadata_name:
                logger.warning(
                    'Generating metadata for package1 %s '
    ...
    def run_egg_info(self):
        ...
        if self.name:
            logger.debug(
                'Running setup.py (path:%s) egg_info for package1 %s',
                self.setup_py, self.name,
            )
        else:
            logger.debug(
                'Running setup.py (path:%s) egg_info for package1 from %s',
